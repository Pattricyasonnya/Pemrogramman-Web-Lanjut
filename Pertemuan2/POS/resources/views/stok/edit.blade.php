  ...
  <div class="card-body">
    ...
    <form method="POST" action="{{ url('/stok/'.$stok->stok_id) }}" class="form-horizontal">
        ...
        <div class="col-11">
          <select class="form-control" id="barang_id" name="barang_id" required>
            ...
            <option value="{{ $item->barang_id }}" @if($item->barang_id ==
              $stok->barang_id) selected @endif>{{ $item->barang_nama }}</option>
            @endforeach
          </select>
          @error('barang_id')
      ...
      <div class="form-group row">
        ...
        <div class="col-11">
          <input type="date" class="form-control" id="stok_tanggal" name="stok_tanggal"
            value="{{ old('stok_tanggal', date('Y-m-d', strtotime($stok->stok_tanggal))) }}">
          @error('stok_tanggal')
          <small class="form-text text-danger">{{ $message }}</small>
          @enderror
        </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-11">
          <input type="number" min="0" class="form-control" id="stok_jumlah" name="stok_jumlah"
      ...
      <div class="form-group row">
        ...
        <div class="col-11">
          ...
          <a class="btn btn-sm btn-default ml-1" href="{{ url('stok') 
      }}">Kembali</a>
        </div>
      </div>
    </form>
